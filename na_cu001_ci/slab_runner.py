...
def load_bulk(handoff_path: Path, result_path: Path) -> dict:
    ...
    if handoff.get("schema") != "na-cu001-bulk-to-slab-handoff-v0.1":
...
def run_case(args: argparse.Namespace) -> None:
    ...
    record = {
        "schema": "na-cu001-clean-slab-case-v0.2", "tag": tag,
    ...
    if proc.returncode or not record["job_done"] or energy_ev is None:
...
def analyze(args: argparse.Namespace) -> None:
    ...
    if any(r.get("schema") != "na-cu001-clean-slab-case-v0.2" for r in rows):
    ...
    for r in rows:
        check = (r["a0_angstrom"], r["e0_ev_per_atom"], r["ecutwfc_ry"], r["ecutrho_ry"],
                 r["handoff_sha256"], r["result_sha256"])
        if check != invariant:
    ...
    result = {
        "schema": "na-cu001-clean-slab-selection-v0.2",
