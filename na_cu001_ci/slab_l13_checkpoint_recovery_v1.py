#!/usr/bin/env python3
"""Checkpoint-aware recovery for the single Na/Cu(001) L13 V16 K22 case.
...
VACUUM = 16.0
KMESH = 22
RECOVERY_SCHEMA = "na-cu001-l13-checkpoint-recovery-v0.1"
...
def case_paths(v5: Any, out: Path) -> tuple[str, Path, Path, Path]:
...
def run_segment(args: argparse.Namespace) -> None:
    if int(args.layers) != LAYERS or float(args.vacuum) != VACUUM or int(args.kmesh) != KMESH:
        raise SystemExit("HOLD: recovery is registered only for L13 V16 K22")
    if int(args.np) != 2:
    ...
    tag, job, record_path, restart_root = case_paths(v5, out)
    ...
    recovery = {
        ...
        "case": {"layers": LAYERS, "vacuum_angstrom": VACUUM, "kmesh_inplane": KMESH},
