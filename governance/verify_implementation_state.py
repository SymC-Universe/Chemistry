#!/usr/bin/env python3
"""Fail-closed verifier for Chemistry deployment and execution state truth."""
...
DEPLOYMENT_STATES = ["FROZEN", "QUALIFIED", "WIRED", "ACTIVE"]
EXECUTION_STATES = ["IDLE", "QUEUED", "RUNNING", "COMPLETED", "FAILED", "HOLD"]
...
def main() -> None:
    ...
    require(reg.get("deployment_state_vocabulary") == DEPLOYMENT_STATES, "deployment-state vocabulary drift")
    require(reg.get("execution_state_vocabulary") == EXECUTION_STATES, "execution-state vocabulary drift")
    ...
    require(qe.get("last_production_run_id") == 33351747179, "QE production run identity drift")
    require(qe.get("last_production_run_conclusion") == "success", "QE production completion drift")
    require(qe.get("execution_state") == "IDLE", "completed QE production incorrectly claims a running execution")
    require(qe.get("last_execution_state") == "COMPLETED", "QE latest execution state drift")
    ...
    require(legacy.get("native_checkpoint_protected") is False, "legacy L15 run incorrectly claims checkpoint protection")
    require(
        legacy.get("successor_identical_unsalvageable_retry_allowed") is False,
        "legacy L15 unsalvageable retry was reopened",
    )
    require(legacy.get("execution_state") == "FAILED", "legacy failed run execution state drift")
    ...
    require(s3.get("execution_state") == "COMPLETED", "System 3 recovery execution state drift")
    ...
    require(flags.get("prior_failure_preserved") is True, "status synchronization lost prior failure")
    ...
    require(ev.get("native_checkpoint_protocol_was_wired_into_run") is False, "historical wiring failure was rewritten")
