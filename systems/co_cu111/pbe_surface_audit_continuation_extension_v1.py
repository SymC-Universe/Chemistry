#!/usr/bin/env python3
"""Fail-closed continuation extension for the frozen CO/Cu(111) L13 audit."""
from __future__ import annotations
import argparse, hashlib, json, math, sys
from pathlib import Path
from typing import Any

SCHEMA = "co-cu111-pbe-surface-audit-continuation-extension-v0.1"
STATUS = "FROZEN_MECHANICAL_CONTINUATION_EXTENSION_AFTER_SEGMENT12_EXHAUSTION"
SEG_SCHEMA = "co-cu111-pbe-surface-audit-continuation-extension-segment-v0.1"
...
def import_parent():
    ...
    import pbe_surface_audit_rank_fallback_recovery_v1 as old
    return old

def protocol(path: Path) -> dict[str, Any]:
    p=load_json(path)
    if p.get("schema")!=SCHEMA or p.get("status")!=STATUS: raise SystemExit("MECHANICAL_HOLD: wrong or unfrozen extension protocol")
    if p.get("scientific_scope")!="NO_SCIENTIFIC_CONFIGURATION_CHANGE": raise SystemExit("MECHANICAL_HOLD: scientific scope changed")
    ex=p["execution"]
    if int(ex.get("maximum_new_continuation_segments",-1))!=12: raise SystemExit("MECHANICAL_HOLD: extension bound changed")
    if ex.get("logical_segment_numbers")!=list(range(13,25)): raise SystemExit("MECHANICAL_HOLD: logical segment sequence changed")
    ...
    s=p["unchanged_science"]
    frozen={"layers":13,"vacuum_angstrom":28.0,"kmesh":24,"ecutwfc_ry":90,"ecutrho_ry":900,"force_gate_ev_per_angstrom":0.02,"independent_scf_reproduction_gate_ev":0.001,"surface_excess_convergence_max_ev_per_surface_atom":0.001}
    ...
    prov=p["provenance"]
    if prov.get("scientific_settings_changed") is not False or prov.get("kinetic_inputs_used") is not False: raise SystemExit("MECHANICAL_HOLD: provenance is not clean")
    return p

def parent_context(args,p):
    old=import_parent()
    oldpp=Path(p["frozen_parent"]["protocol_path"]).resolve(); oldrp=Path(p["frozen_parent"]["runner_path"]).resolve()
    if sha256(oldpp)!=p["frozen_parent"]["protocol_sha256"]: raise SystemExit("MECHANICAL_HOLD: parent protocol hash mismatch")
    if sha256(oldrp)!=p["frozen_parent"]["runner_sha256"]: raise SystemExit("MECHANICAL_HOLD: parent runner hash mismatch")
    oldp=old.protocol(oldpp)
    base,relay,surface,bundle=old.runtime_context(args,oldp)
    sp=Path(args.selection).resolve()
    if sha256(sp)!=p["selection"]["selection_sha256"]: raise SystemExit("MECHANICAL_HOLD: rank-selection hash mismatch")
    sel=old.load_selection(sp,oldp)
    if int(sel.get("selected_mpi_ranks",-1))!=1 or sel.get("selected_execution_mode")!="DIRECT_ONE_RANK": raise SystemExit("MECHANICAL_HOLD: extension requires frozen direct one-rank mode")
    return old,oldp,base,relay,surface,bundle,sel

def verify_parent_segment12(root: Path,p,old):
    q=find_one(root,"RANK_FALLBACK_SEGMENT.json")
    if sha256(q)!=p["parent_run"]["segment12_record_sha256"]: raise SystemExit("MECHANICAL_HOLD: parent segment-12 record hash mismatch")
    d=load_json(q); e=p["parent_run"]
    for k,v in {"status":e["required_status"],"segment":e["required_segment"],"logical_segment":e["required_logical_segment"],"execution_mode":e["required_execution_mode"],"mpi_ranks":e["required_mpi_ranks"],"pw_sha256":e["pw_sha256"],"protocol_sha256":e["parent_protocol_sha256"],"rank_selection_sha256":e["rank_selection_sha256"]}.items():
        if d.get(k)!=v: raise SystemExit(f"MECHANICAL_HOLD: parent segment-12 mismatch: {k}")
    if abs(float(d["latest_authoritative_max_movable_force_ev_per_angstrom"])-float(e["latest_authoritative_max_movable_force_ev_per_angstrom"]))>1e-12: raise SystemExit("MECHANICAL_HOLD: segment-12 force drift")
    if abs(float(d["energy_ev"])-float(e["energy_ev"]))>1e-10: raise SystemExit("MECHANICAL_HOLD: segment-12 energy drift")
    if d.get("raw_hashes",{}).get("relax_input_sha256")!=e["raw_relax_input_sha256"] or d.get("raw_hashes",{}).get("relax_output_sha256")!=e["raw_relax_output_sha256"]: raise SystemExit("MECHANICAL_HOLD: segment-12 raw hash drift")
    ...
    if not d.get("next_trial_atoms"): raise SystemExit("MECHANICAL_HOLD: segment-12 lacks next BFGS trial")
    disp=float(d.get("source_evidence",{}).get("next_trial_displacement_angstrom",math.nan))
    if not math.isfinite(disp) or abs(disp-float(e["next_trial_displacement_angstrom"]))>1e-12: raise SystemExit("MECHANICAL_HOLD: segment-12 proposal displacement drift")
    return d,q

def load_prior(root,p,old,segment):
    if segment==1:
        d,q=verify_parent_segment12(root,p,old)
        return d["next_trial_atoms"],{"source":"parent_rank_fallback_segment_12","source_record_sha256":sha256(q),"source_raw_output_sha256":d["raw_hashes"]["relax_output_sha256"]},False
    q=find_one(root,"CONTINUATION_EXTENSION_SEGMENT.json"); d=load_json(q)
    if d.get("schema")!=SEG_SCHEMA or int(d.get("segment",-1))!=segment-1: raise SystemExit("MECHANICAL_HOLD: prior extension segment mismatch")
    if d.get("status")=="RELAX_COMPLETE": return d["final_atoms"],{"source":"prior_relax_complete","source_record_sha256":sha256(q)},True
    if d.get("status")!="CONTINUE" or not d.get("next_trial_atoms"): raise SystemExit("MECHANICAL_HOLD: invalid prior extension state")
    return d["next_trial_atoms"],{"source":"prior_next_trial","source_record_sha256":sha256(q)},False

def command_self_test(args):
    protocol(Path(args.protocol).resolve()); print("CONTINUATION_EXTENSION_SELF_TEST_PASS"); print("DIRECT_ONE_RANK_FROZEN=true"); print("SCIENTIFIC_SETTINGS_CHANGED=false"); print("KINETIC_INPUTS_USED=false")

def command_verify_parent(args):
    p=protocol(Path(args.protocol).resolve()); old=import_parent(); d,q=verify_parent_segment12(Path(args.parent_root).resolve(),p,old)
    print(json.dumps({"status":"PARENT_SEGMENT12_VERIFIED","record_sha256":sha256(q),"latest_force_ev_per_angstrom":d["latest_authoritative_max_movable_force_ev_per_angstrom"],"next_trial_displacement_angstrom":d["source_evidence"]["next_trial_displacement_angstrom"]},sort_keys=True))

def command_continue(args):
    pp=Path(args.protocol).resolve(); p=protocol(pp); old,oldp,base,relay,surface,bundle,sel=parent_context(args,p)
    segment=int(args.segment)
    if segment<1 or segment>int(p["execution"]["maximum_new_continuation_segments"]): raise SystemExit("MECHANICAL_HOLD: extension segment outside frozen bound")
    seed,evidence,already=load_prior(Path(args.prior_root).resolve(),p,old,segment)
    ...
    if already:
        src=load_json(find_one(Path(args.prior_root).resolve(),"CONTINUATION_EXTENSION_SEGMENT.json")); carried=dict(src); carried.update({"segment":segment,"logical_segment":12+segment,"carried_forward_without_recomputation":True,"source_evidence":evidence})
        write_json(root/"CONTINUATION_EXTENSION_SEGMENT.json",carried); base.stage_manifest(root,[root/"CONTINUATION_EXTENSION_SEGMENT.json"]); print(json.dumps(carried,indent=2,sort_keys=True)); return
    run_dir=root/"relax"; run_dir.mkdir(exist_ok=True); tmp=run_dir/"tmp"; tmp.mkdir(exist_ok=True); inp=run_dir/"clean_relax.in"; out=run_dir/"clean_relax.out"
    ...
    if complete:
        ...
        if proposal is None: raise SystemExit("MECHANICAL_HOLD: bounded extension emitted no admissible next BFGS trial")
        ...
        if delta<=1e-10: raise SystemExit("MECHANICAL_HOLD: extension would repeat the same geometry")
        evidence["next_trial_parent_evidence"]=pe; evidence["next_trial_displacement_angstrom"]=delta
    row={"schema":SEG_SCHEMA,"status":"RELAX_COMPLETE" if complete else "CONTINUE","segment":segment,"logical_segment":12+segment,"case_id":"L13-V28-K24-audit","mpi_ranks":1,"execution_mode":"DIRECT_ONE_RANK","runner_label":p["execution"]["runner_label"],"thread_caps":p["execution"]["thread_caps"],"timed_out_by_wrapper":result["timed_out_by_wrapper"],"pw_returncode":result["returncode"],"job_done":result["job_done"],"bfgs_finished":result["bfgs_finished"],"energy_ev":result["energy_ev"],"latest_authoritative_max_movable_force_ev_per_angstrom":latest,"input_atoms":seed,"final_atoms":final_atoms,"next_trial_atoms":next_trial,"source_evidence":evidence,"scientific_settings_changed":False,"parallelization_changed":False,"kinetic_inputs_used":False,"raw_hashes":{"relax_input_sha256":sha256(inp),"relax_output_sha256":sha256(out)},"extension_protocol_sha256":sha256(pp),"parent_protocol_sha256":p["frozen_parent"]["protocol_sha256"],"rank_selection_sha256":sha256(Path(args.selection).resolve()),"surface_protocol_sha256":oldp["frozen_sources"]["surface_protocol"]["sha256"],"pw_sha256":sha256(Path(args.pw).resolve()),"elapsed_s":result["elapsed_s"]}
    base.cleanup_tmp(tmp); write_json(root/"CONTINUATION_EXTENSION_SEGMENT.json",row); base.stage_manifest(root,[root/"CONTINUATION_EXTENSION_SEGMENT.json"]); print(json.dumps(row,indent=2,sort_keys=True))

def command_reproduce(args):
    pp=Path(args.protocol).resolve(); p=protocol(pp); old,oldp,base,relay,surface,bundle,sel=parent_context(args,p)
    seg_path=find_one(Path(args.prior_root).resolve(),"CONTINUATION_EXTENSION_SEGMENT.json"); seg=load_json(seg_path)
    if seg.get("status")!="RELAX_COMPLETE" or not seg.get("final_atoms"): raise SystemExit("MECHANICAL_INCOMPLETE: L13 audit relaxation still incomplete after twelve extension segments")
    if int(seg.get("mpi_ranks",-1))!=1 or seg.get("execution_mode")!="DIRECT_ONE_RANK": raise SystemExit("MECHANICAL_HOLD: final extension execution mode drift")
